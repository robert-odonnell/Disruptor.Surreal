...
public class SurrealRetryPolicyTests
{
    [Fact]
    public async Task SucceedsOnFirstAttempt_NoRetry()
    {
        var attempts = 0;
        var result = await SurrealRetryPolicy.WithRetryAsync((_, _) =>
        {
            attempts++;
    ...
    {
        var attempts = 0;
        var result = await SurrealRetryPolicy.WithRetryAsync<string>((n, _) =>
        {
            attempts++;
    ...
    public async Task PropagatesConflict_AfterMaxAttempts()
    {
        ...
        await Assert.ThrowsAsync<SurrealConflictException>(() =>
            SurrealRetryPolicy.WithRetryAsync<int>((_, _) =>
            {
                attempts++;
    ...
    {
        var attempts = 0;
        await SurrealRetryPolicy.WithRetryAsync((n, _) =>
        {
            attempts++;
