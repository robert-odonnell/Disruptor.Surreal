...
/// <see cref="SurrealGeometry.Point"/> / <see cref="SurrealGeometry.Line"/> / etc.
/// </remarks>
public abstract record SurrealGeometry
{
    private protected SurrealGeometry() { }

    /// <summary>A 2D point (x, y).</summary>
    public sealed record Point(double X, double Y) : SurrealGeometry
    {
    ...
    public sealed record Line : SurrealGeometry
    {
        public IReadOnlyList<Point> Points { get; }

        public Line(IEnumerable<Point> points) =>
            Points = points as IReadOnlyList<Point> ?? [.. points];
    ...
    public sealed record Polygon : SurrealGeometry
    {
        ...
        public Polygon(Line exterior, IEnumerable<Line>? interiors = null)
        {
            ...
            Interiors = interiors is null
                ? []
                : interiors as IReadOnlyList<Line> ?? [.. interiors];
        }
    ...
    public sealed record MultiPoint : SurrealGeometry
    {
        public IReadOnlyList<Point> Points { get; }
    ...
    public sealed record MultiLine : SurrealGeometry
    {
        public IReadOnlyList<Line> Lines { get; }
        public MultiLine(IEnumerable<Line> lines) =>
            Lines = lines as IReadOnlyList<Line> ?? [.. lines];
    ...
    public sealed record MultiPolygon : SurrealGeometry
    {
        public IReadOnlyList<Polygon> Polygons { get; }
        public MultiPolygon(IEnumerable<Polygon> polygons) =>
            Polygons = polygons as IReadOnlyList<Polygon> ?? [.. polygons];
    ...
    public sealed record Collection : SurrealGeometry
    {
        public IReadOnlyList<SurrealGeometry> Items { get; }
        public Collection(IEnumerable<SurrealGeometry> items) =>
            Items = items as IReadOnlyList<SurrealGeometry> ?? [.. items];
