...
/// transport layer for unit-testing code that holds a <see cref="SurrealClient"/>; the
...
/// <c>"signin"</c>, <c>"begin"</c>, …) and params is whatever <see cref="SurrealValue"/>
/// payload that method expects (typically an <see cref="SurrealListValue"/>).
/// </remarks>
public interface ISurrealConnection : IAsyncDisposable
{
    ...
    /// <paramref name="params"/> is the wire-level <see cref="SurrealValue"/> payload (usually
    /// an <see cref="SurrealListValue"/>) or <c>null</c> when the method takes none;
    ...
    Task<SurrealValue> SendAsync(string method, SurrealValue? @params, Guid? txnId, CancellationToken ct = default);
    ...
    /// request is retried once. Set by <see cref="SurrealClient"/> after a successful signin.
    /// </summary>
    ...
    /// </summary>
    void RegisterLiveSubscription(Guid liveQueryId, ChannelWriter<SurrealNotification> writer);

    /// <summary>
    /// Remove a live subscription. Called by <see cref="SurrealLiveQueryHandle"/> on disposal
